...
import { SuiHTTPTransport } from './http-transport';
...
export class SuiClient {
  ...
  constructor(options: SuiClientOptions) {
    this.transport =
      options.transport ?? new SuiHTTPTransport({ url: options.url });
  }
  ...
  >): Promise<SuiTransactionBlockResponse> {
    ...
    if (transactionBlock instanceof Uint8Array) {
      ...
      transactionBlock.setSenderIfNotSet(
        await signer.getPublicKey().toSuiAddress(),
      );
      transactionBytes = await transactionBlock.build({ client: this });
    }

    const { signature, bytes } = await signer.signTransactionBlock(
      transactionBytes,
    );
  ...
  async getCheckpoints(
    input: PaginationArguments<CheckpointPage['nextCursor']> &
      GetCheckpointsParams,
