...
export class MultiSigPublicKey extends PublicKey {
  ...
  constructor(
    ...
    this.publicKeys = this.multisigPublicKey.pk_map.map(
      ({ pubKey, weight }) => {
        const [scheme, bytes] = Object.entries(pubKey)[0] as [
          SignatureScheme,
          number[],
        ];
        return {
  ...
  static fromPublicKeys({
    ...
    return new MultiSigPublicKey({
      pk_map: publicKeys.map(({ publicKey, weight }) => {
        const scheme =
          SIGNATURE_FLAG_TO_SCHEME[publicKey.flag() as SignatureFlag];
  ...
  async verify(
    ...
    const { signatureScheme, multisig } =
      parseSerializedSignature(multisigSignature);
    ...
    for (const { publicKey, weight, signature } of parsePartialSignatures(
      multisig,
    )) {
  ...
  combinePartialSignatures(
    ...
    let bitmap = 0;
    const compressedSignatures: CompressedSignature[] = new Array(
      signatures.length,
    );
    ...
    for (let i = 0; i < signatures.length; i++) {
      ...
      for (let j = 0; j < this.publicKeys.length; j++) {
        if (
          bytesEqual(
            parsed.publicKey,
            this.publicKeys[j].publicKey.toRawBytes(),
          )
        ) {
          if (bitmap & (1 << j)) {
            throw new Error(
...
export function parsePartialSignatures(
  ...
  for (let i = 0; i < multisig.sigs.length; i++) {
    const [signatureScheme, signature] = Object.entries(
      multisig.sigs[i],
