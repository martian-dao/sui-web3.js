...
async function setupAccount() {
  ...
  try {
    ...
    console.log(err);
    const mnemonic =
      'arena nothing skate then sport huge fence era cheese client powder tackle';
...
//     },
//   };
//   const response = await apis.simulateTransaction(
//     aliceAccount.getPublicKey().toSuiAddress(),
//     txn
//   );
...
//   const response: Wallet = await apis.importWallet(
//     'arena nothing skate then sport huge fence era cheese client powder tackle'
//   );
...
test('verify getNfts', async () => {
  const response: any = await apis.getNfts("0x6dcd3d1023b975f68e6a8f9a854498bda9720b59");
